...
plan: "03"
subsystem: generator
tags: [go-generator, yang, rpc, action, notification, struct-generation]

# Dependency graph
requires:
  - phase: 03-go-generator/03-02
    provides: generator foundation with AnyData/AnyXML and prior struct generation
provides:
...
affects:
  - 04-protobuf-generator (protobuf service generation for RPCs follows same schema types)
  - Future callers building gRPC/REST APIs from YANG schema

# Tech tracking
tech-stack:
  added: []
  patterns:
    - Post-loop pass pattern for single-emission of cross-cutting constructs
    - generateField skip pattern for non-data schema nodes (RPC/Action/Notification)
    - Separate child-type pass in generateStruct for RFC 7950 §7.15 Action handling
