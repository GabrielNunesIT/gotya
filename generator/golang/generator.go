...
func (g *GoGenerator) GenerateDevice(modules []*schema.Module, w io.Writer) error {
	...
	for _, treeType := range []string{"Config", "State"} {
		...
		if g.Options.GenerateFakeroot {
			...
			if g.Options.AddAnnotations {
				if _, err := fmt.Fprintf(w, "\tYANGAnnotations map[string]interface{} `json:\"@,omitempty\"`\n"); err != nil {
					return fmt.Errorf("write err: %w", err)
				}
			}
...
func (g *GoGenerator) generateStruct(name string, description *string, children map[string]schema.Node, kind string, w io.Writer, visited map[string]bool, prefix, suffix, namespace string, configOnly bool) error {
	...
	if g.Options.GenerateGetters {
		if err := g.generateGetters(structName, validFlatNodes, prefix, suffix, namespace, w); err != nil {
			return err
		}
	}
	if g.Options.GenerateSetters {
		if err := g.generateSetters(structName, validFlatNodes, prefix, suffix, namespace, w); err != nil {
			return err
		}
	}
	if g.Options.GeneratePopulateDefault {
		if err := g.generatePopulateDefault(structName, validFlatNodes, prefix, suffix, namespace, w); err != nil {
			return err
		}
	}
	if g.Options.GenerateOrderedMaps {
		if err := g.generateSorts(structName, validFlatNodes, prefix, suffix, namespace, w); err != nil {
			return err
		}
	}
...
func (g *GoGenerator) generateField(node schema.Node, w io.Writer, prefix, suffix, namespace string) error {
	...
	case *schema.List:
		if g.Options.GenerateOrderedMaps {
			...
			goType = "map[string]*" + prefix + toCamelCaseTitle(n.Name()) + "Entry" + suffix
		}
	}
...
// generateSorts generates sorting methods for list and leaf-list slices that are ordered by system.
func (g *GoGenerator) generateSorts(structName string, nodes []schema.Node, prefix, suffix, namespace string, w io.Writer) error {
	for _, node := range nodes {
		...
		case *schema.List:
			if !g.Options.GenerateOrderedMaps {
				continue // map-based lists don't need sorting
			}
			...
			if orderedBy == "user" {
				continue // user order is preserved as insertion order
			}
			...
			if _, err := fmt.Fprintf(w, "// Sort%s sorts the %s slice according to its YANG ordered-by semantics (system-ordered by keys).\n", fieldName, fieldName); err != nil {
				return err
			}
			if _, err := fmt.Fprintf(w, "func (s *%s) Sort%s() {\n", structName, fieldName); err != nil {
				return err
			}
			if _, err := fmt.Fprintf(w, "\tif len(s.%s) <= 1 {\n\t\treturn\n\t}\n", fieldName); err != nil {
				return err
			}

			if _, err := fmt.Fprintf(w, "\tsort.SliceStable(s.%s, func(i, j int) bool {\n", fieldName); err != nil {
				return err
			}

			for _, key := range n.Keys {
				keyFieldName := toCamelCaseTitle(key)
				if _, err := fmt.Fprintf(w, "\t\tif s.%s[i].%s != nil && s.%s[j].%s != nil {\n", fieldName, keyFieldName, fieldName, keyFieldName); err != nil {
					return err
				}
				if _, err := fmt.Fprintf(w, "\t\t\tif *s.%s[i].%s != *s.%s[j].%s {\n", fieldName, keyFieldName, fieldName, keyFieldName); err != nil {
					return err
				}
				if _, err := fmt.Fprintf(w, "\t\t\t\treturn *s.%s[i].%s < *s.%s[j].%s\n", fieldName, keyFieldName, fieldName, keyFieldName); err != nil {
					return err
				}
				if _, err := fmt.Fprintf(w, "\t\t\t}\n"); err != nil {
					return err
			...
			if _, err := fmt.Fprintf(w, "\t\treturn false\n"); err != nil {
				return err
			}
			if _, err := fmt.Fprintf(w, "\t})\n"); err != nil {
				return err
		...
		case *schema.LeafList:
			...
			goType, _, _ := g.resolveFieldTypeInfo(n, prefix, suffix)
			
			if _, err := fmt.Fprintf(w, "// Sort%s sorts the %s leaf-list according to its YANG ordered-by semantics (system-ordered by value).\n", fieldName, fieldName); err != nil {
				return err
			}
			if _, err := fmt.Fprintf(w, "func (s *%s) Sort%s() {\n", structName, fieldName); err != nil {
				return err
			...
			if !strings.Contains(goType, "Union") && !strings.Contains(goType, "bool") {
				if _, err := fmt.Fprintf(w, "\t\tif s.%s[i] != nil && s.%s[j] != nil {\n\t\t\treturn *s.%s[i] < *s.%s[j]\n\t\t}\n\t\treturn false\n", fieldName, fieldName, fieldName, fieldName); err != nil {
					return err
				}
			} else {
				if _, err := fmt.Fprintf(w, "\t\treturn false // Cannot natively sort Union or bool values natively\n"); err != nil {
					return err
				}
			}
