...
func (g *GoGenerator) GenerateDevice(modules []*schema.Module, w io.Writer) error {
	// Buffer all output so we can run go/format.Source on the complete source
	// before writing to the caller's writer. This catches any syntax errors in
	// generated Go at generation time rather than silently emitting bad output.
	var buf bytes.Buffer

	if _, err := fmt.Fprintf(&buf, "// Code generated by gotya. DO NOT EDIT.\npackage %s\n\nimport (\n\t\"encoding/json\"\n\t\"encoding/xml\"\n\t\"fmt\"\n\t\"regexp\"\n\t\"sort\"\n\t\"strings\"\n\t\"unicode/utf8\"\n)\n\nvar (\n\t_ = json.Marshal\n\t_ = xml.Unmarshal\n\t_ = fmt.Errorf\n\t_ = regexp.MatchString\n\t_ = sort.SliceStable\n\t_ = strings.Join\n\t_ = utf8.RuneCountInString\n)\n\n", g.Options.PackageName); err != nil {
		return fmt.Errorf("failed to write package header: %w", err)
	}
	...
	for _, treeType := range []string{"Config", "State"} {
		...
		if g.Options.GenerateFakeroot {
			if _, err := fmt.Fprintf(&buf, "// %s%s represents the aggregated YANG configuration.\n", g.Options.RootName, treeType); err != nil {
				return fmt.Errorf("write err: %w", err)
			}
			if _, err := fmt.Fprintf(&buf, "type %s%s struct {\n", g.Options.RootName, treeType); err != nil {
				return fmt.Errorf("write err: %w", err)
			}
			...
			for _, mod := range modules {
				...
				modFieldName := toCamelCaseTitle(mod.Name)
				if _, err := fmt.Fprintf(&buf, "\tif s.%s != nil {\n\t\tif err := s.%s.Validate(); err != nil {\n\t\t\treturn err\n\t\t}\n\t}\n", modFieldName, modFieldName); err != nil {
					return fmt.Errorf("write err: %w", err)
				}
			}
			if _, err := fmt.Fprintf(&buf, "\treturn nil\n}\n\n"); err != nil {
				return fmt.Errorf("write err: %w", err)
			}
		}
		...
		for _, mod := range modules {
			...
			if _, err := fmt.Fprintf(&buf, "// %s represents the YANG module %s.\n", modStructName, mod.Name); err != nil {
				return fmt.Errorf("write err: %w", err)
			}
			if _, err := fmt.Fprintf(&buf, "type %s struct {\n", modStructName); err != nil {
				return fmt.Errorf("write err: %w", err)
			}
			...
			if err := g.generateValidationChecks(modStructName, modTopNodes, &buf); err != nil {
				return err
			}
			if g.Options.GenerateGetters {
				if err := g.generateGetters(modStructName, modTopNodes, prefix, treeType, &buf); err != nil {
					return err
				}
			}
			if g.Options.GenerateSetters {
				if err := g.generateSetters(modStructName, modTopNodes, prefix, treeType, &buf); err != nil {
					return fmt.Errorf("generateSetters err: %w", err)
				}
			}
			if g.Options.GeneratePopulateDefault {
				if err := g.generatePopulateDefault(modStructName, modTopNodes, prefix, treeType, &buf); err != nil {
					return fmt.Errorf("generatePopulateDefault err: %w", err)
				}
			}
			if g.Options.GenerateOrderedMaps {
				if err := g.generateSorts(modStructName, modTopNodes, &buf); err != nil {
					return err
				}
			}
